...
class EngagementLetter(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            seq_code = self._get_sequence_code_for_department(department_type)
            sequence_date = self._get_reference_sequence_date(vals)
            vals["reference"] = records.with_context(ir_sequence_date=sequence_date).next_by_code(seq_code)
    ...
    def _ensure_can_approve(self):
        if not self.env.user.has_group("qlk_engagement_letter.group_engagement_manager"):
    ...
    def action_print_letter(self):
        ...
        return self.env.ref("qlk_engagement_letter.action_report_engagement_letter").report_action(self)

    def _default_approver(self):
        manager_group = self.env.ref("qlk_engagement_letter.group_engagement_manager", raise_if_not_found=False)
    ...
    def _notify_corporate_overage(self, month, year, hours_over):
        partners = self._notification_partners()
        template = self.env.ref("qlk_engagement_letter.mail_template_engagement_corporate_overage", raise_if_not_found=False)
        if template:
    ...
    def _notify_litigation_overage(self):
        partners = self._notification_partners()
        template = self.env.ref("qlk_engagement_letter.mail_template_engagement_litigation_overage", raise_if_not_found=False)
        if template:
            ctx = {
...
class EngagementInstallment(models.Model):
    ...
    def _notify_upcoming_payment(self):
        ...
        template = self.env.ref("qlk_engagement_letter.mail_template_engagement_installment_due", raise_if_not_found=False)
        if template:
            ctx = {
