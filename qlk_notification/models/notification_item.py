...
class NotoNotificationItem(models.Model):
    ...
    reference_field_name = fields.Char(
        string="Reminder Field",
        help="Technical field name used to push snooze/done updates back to the source record.",
    )
    ...
    def action_mark_done(self):
        for record in self:
            ...
            record.write(values)
            record._update_reference_record(False)
        return True
    ...
    def action_snooze(self, minutes=15):
        ...
        for record in self:
            ...
            record._update_reference_record(new_time)
        return True
    ...
    # Internal utilities
    # -------------------------------------------------------------------------
    def _update_reference_record(self, value):
        self.ensure_one()
        if not (
            self.reference_model_id
            and self.reference_res_id
            and self.reference_field_name
        ):
            return
        try:
            record = self.env[self.reference_model_id.model].browse(self.reference_res_id)
        except Exception:
            return
        if not record.exists():
            return
        if self.reference_field_name not in record._fields:
            return
        context = dict(self.env.context, noto_skip_sync=True)
        try:
            record.with_context(context).write({self.reference_field_name: value})
        except Exception:
            # avoid breaking notification flow if source record rejects the write
            return
