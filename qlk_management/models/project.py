...
class QlkProject(models.Model):
    ...
    legal_fee_line_ids = fields.One2many(
        "qlk.project.fee",
        "project_id",
    ...
    def _compute_client_code(self):
        for project in self:
            ...
            if project.client_id:
                code = project.client_id.code or project.client_id.ref or f"{project.client_id.id:03d}"
        ...
        for vals in vals_list:
            ...
            if not prepared.get("name"):
                ...
                sequence = self.env["ir.sequence"].with_company(company_id).next_by_code(
                    "qlk.project.sequence"
                )
                sequence = sequence or "PROJ001"
                client_code = ""
                client_id = prepared.get("client_id")
                if client_id:
                    partner = self.env["res.partner"].browse(client_id)
                    client_code = partner.code or partner.ref or ""
                prepared["name"] = f"{client_code}/{sequence}" if client_code else sequence
    ...
    def action_view_hours(self):
        self.ensure_one()
        action = self.env.ref("qlk_management.action_qlk_project_hours").read()[0]
        action["domain"] = [
            ("project_id", "=", self.id),
...
class QlkProjectFee(models.Model):
    _name = "qlk.project.fee"
    _description = "Project Fee Line"
