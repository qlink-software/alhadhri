...
class ArbitrationCaseProject(models.Model):
    ...
    def _onchange_engagement_id(self):
        for record in self:
            ...
            if not engagement:
                continue
            if engagement.partner_id and not record.claimant_id:
                record.claimant_id = engagement.partner_id.id
    ...
    def _onchange_project_id(self):
        for record in self:
            ...
            if project.client_id and not record.claimant_id:
                record.claimant_id = project.client_id.id
    ...
    def create(self, vals_list):
        ...
        if not self.env.context.get("skip_engagement_service_validation"):
            for vals in vals_list:
                ...
                if engagement and engagement.service_type not in ("arbitration", "mixed"):
                    raise UserError(_("This engagement letter does not allow arbitration records."))
    ...
    def _apply_project_defaults(self, vals):
        ...
        if project.exists():
            ...
            vals.setdefault("claimant_id", project.client_id.id)
    ...
    def _check_project_service_rules(self):
        for record in self:
            if not record.project_id:
                raise ValidationError(_("Arbitration records must be created from a project."))
            if record.project_id.service_type != "arbitration":
