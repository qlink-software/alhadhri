...
class LawyerDashboardRequests(models.AbstractModel):
    ...
    @api.model
    def _request_action_payload(self):
        action = self.env.ref("qlk_requests.action_qlk_request", raise_if_not_found=False)
    ...
    @api.model
    def _get_request_bucket(self, domain, lang):
        Request = self.env["qlk.internal.request"]
        action = self._request_action_payload()
        breakdown = {}
    ...
    def _get_requests_dashboard_payload(self, lang):
        ...
        return {
            "my": self._get_request_bucket(my_domain, lang),
            "assigned": self._get_request_bucket(assigned_domain, lang),
        }
